...
describe('Backend Test Spec', () => {
  ...
  it('creating a presentation with invalid data should result with 400 status code', () => {
    cy.request({
      ...
      assert.equal(response.status, 400, "Response status should be 400 when creating a template with no polls")
    })
  })
    ...
    cy.request({
      ...
      cy.request({
        ...
        cy.request({
          ...
          assert.equal(response.status, 200, 'getting current poll should return 200 status code')
          assert.isDefined(response.body.poll_id, 'poll_id should be returned')
          assert.equal(response.body.question, "Which of the countries would you like to visit the most?")
          assert.isUndefined(response.body.votes, 'votes should not be returned')
          ...
          cy.request({
            ...
            assert.equal(response.status, 200, 'reading presentation votes should return 200')
            ...
            cy.request({
              ...
              assert.equal(response.status, 204, 'voting should return 204 status code')
              ...
              cy.request({
                ...
                assert.equal(response.status, 200, 'reading votes should return 200 status code')
